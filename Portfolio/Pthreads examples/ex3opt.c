...
int main(int argc, char* argv[]) {
    ...
    padded_arr* counter_arr= calloc(thread_count, sizeof(padded_arr));
    int err;
    ...
    for(int i= 0; i< thread_count; i++) {
        if((err= pthread_create(&threads[i], NULL, thread_work, &counter_arr[i].counter)) != 0) {
    ...
    int sum= 0;
    for(int i= 0; i< thread_count; i++) {
        printf("%d ", counter_arr[i].counter);
        sum+= counter_arr[i].counter;
    }
    printf("\nTotal sum: %d\n", sum);
