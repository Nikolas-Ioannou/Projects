...
typedef struct {
    ...
    int maxrec; //How many records can be stored in a block
} HP_info;

typedef struct {
    bool hasrec[BF_BLOCK_SIZE/sizeof(Record)]; //This will show if we have a record in a specific place on the block(true means yes and false means no)
}HP_block_info;//This contains the information of the block
